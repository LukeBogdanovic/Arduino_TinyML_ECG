...
void computeFFT()
{
  ...
  for(size_t i = 0; i < FFT_BINS; i++)
  {
    ...
    if(mag < 0) mag = 0; // Clamp value if below 0
    if(mag > 65535.0) mag = 65535.0; // clamp value if above 65535
...
void loop()
{
  if (sampleNow) { // Check sampling flag
  ...
  if (bufferReady) { // Check if buffer is ready
